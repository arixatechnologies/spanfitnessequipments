...
import { business, categories, navItems, phoneDisplay, whatsappUrl } from "../data";
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#09122c]/95 shadow-[0_18px_55px_rgba(0,0,0,.32)] backdrop-blur-xl">
      <div className="hidden border-b border-white/5 bg-white/[0.025] sm:block">
        <div className="section-shell flex h-9 items-center justify-between text-[11px] font-semibold text-white/60">
          <div className="flex items-center gap-5"><a href={`tel:${business.phone}`} className="flex items-center gap-2 hover:text-coral"><Phone className="size-3.5 text-coral" /> {phoneDisplay}</a><span className="hidden items-center gap-2 md:flex"><MapPin className="size-3.5 text-coral" /> Visakhapatnam, Andhra Pradesh</span></div>
      ...
      <nav className="section-shell flex min-h-[84px] items-center justify-between gap-4">
        <Link href="/" aria-label="Span Fitness home"><Image src="/span-fitness-logo-light.png" alt="Span Fitness Equipments" width={192} height={56} priority className="h-14 w-40 object-contain object-left 2xl:w-48" /></Link>
        <div className="hidden items-center xl:flex">
          {navItems.map(([label, href]) => {
            const active = href === "/" ? path === "/" : path.startsWith(href);
            return <Link key={href} href={href} className={`group relative px-1.5 py-4 text-[11px] font-bold transition hover:text-white 2xl:px-2.5 2xl:text-xs ${active ? "text-white" : "text-white/60"}`}>{label}<span className={`absolute inset-x-1.5 bottom-2 h-0.5 origin-left bg-coral transition-transform duration-300 2xl:inset-x-2.5 ${active ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`} /></Link>;
          })}
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/contact" className="btn-shine hidden rounded-md bg-gradient-to-r from-ember to-coral px-5 py-3 text-sm font-bold shadow-[0_9px_28px_rgba(135, 35, 65,.3)] 2xl:block">Get a Quote</Link>
...
export function Footer({ categories: footerCategories = categories }: { categories?: Category[] }) {
  ...
  if (path.startsWith("/admin")) return null;
  return (
    <footer className="site-footer relative overflow-hidden border-t border-white/10 bg-[#09122c] pt-10">
      <div className="site-footer__glow site-footer__glow--one" />
      <div className="site-footer__glow site-footer__glow--two" />
      <div className="section-shell relative grid gap-4 pb-7 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-[1.35fr_1.7fr_1.2fr_.95fr_1.45fr]">
        <div className="site-footer__brand">
          ...
          <p className="mt-4 text-sm leading-6 text-white/55">Premium cardio, strength, home gym, multi play station and commercial fitness solutions across Andhra Pradesh and Telangana.</p>
        </div>
        <div className="site-footer__panel">
          <h3>Explore</h3>
          <div className="site-footer__links site-footer__links--explore mt-3">
            {navItems.map(([label, href]) => (
        ...
        <div className="site-footer__panel">
          <h3>Categories</h3>
          <div className="site-footer__links mt-3 grid gap-1.5">
            {footerCategories.slice(0, 5).map((item) => (
              <Link key={item.slug} href={`/categories/${item.slug}`}>
                <span className="site-footer__dot" />
                <span>{item.name}</span>
              </Link>
            ))}
            <Link href="/products/multi-play-station-equipment">
              <span className="site-footer__dot" />
        ...
        <div className="site-footer__panel">
          ...
          <div className="mt-3 grid gap-2 text-sm text-white/70">
            <a href={`tel:${business.phone}`} className="site-footer__contact"><Phone className="size-4 shrink-0 text-coral" />{phoneDisplay}</a>
...
export function SectionTitle({ eyebrow, title, body }: { eyebrow: string; title: string; body?: string }) {
  return <div className="mb-8 max-w-3xl"><p className="text-xs font-black uppercase tracking-[0.2em] text-coral">{eyebrow}</p><h2 className="mt-3 font-display text-3xl font-black sm:text-5xl">{title}</h2>{body && <p className="mt-3 leading-7 text-white/60">{body}</p>}</div>;
}
...
export function CtaBand({ title = "Planning a new fitness space?", text = "Tell us about your space, users and budget. We will help shortlist the right equipment mix." }: { title?: string; text?: string }) {
  return <section className="py-12"><div className="section-shell rounded-3xl border border-coral/30 bg-gradient-to-br from-wine/65 to-[#101a38] p-7 sm:p-10"><div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div><h2 className="font-display text-3xl font-black">{title}</h2><p className="mt-3 max-w-2xl text-white/65">{text}</p></div><div className="flex flex-wrap gap-3"><ButtonLink href="/contact">Request a Quote <ArrowRight className="size-4" /></ButtonLink><ButtonLink href={whatsappUrl()} secondary>WhatsApp Us</ButtonLink></div></div></div></section>;
}
